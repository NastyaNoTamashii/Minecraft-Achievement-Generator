# Minecraft Achievement Generator

A simple Python script that lets you create images with any achievements in Minecraft style.

___
![image](https://github.com/NastyaNoTamashii/Minecraft-Achievement-Generator/blob/main/achievements/pizza_time!.png?raw=true)
___
![image](https://github.com/NastyaNoTamashii/Minecraft-Achievement-Generator/blob/main/preview.png?raw=true)

## Download using git

```
git clone https://github.com/NastyaNoTamashii/Minecraft-Achievement-Generator.git
```

## Usage

1. If you don't have python installed, download python and make sure you select the 'ADD TO PATH' option during installation.

2. Install required modules > `pip install -r requirements.txt`

3. Enter `python3 main.py` in terminal for script run

4. In the terminal
## Notation
